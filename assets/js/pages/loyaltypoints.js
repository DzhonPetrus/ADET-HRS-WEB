$(function () {
	window.fields = ["loyalty_point_id", "points","creator", "btnAdd", "btnUpdate"];
	window.fieldsHidden = ["loyalty_point_id", "creator", "btnUpdate"];
	window.readOnlyFields = ["loyalty_point_id", "creator"];

	formReset();
	loadTable();

	// function to save/update record
	$("#loyalty_point_id_form").on("submit", function (e) {
		e.preventDefault();
		trimInputFields();

		if ($("#loyalty_point_id_form").parsley().validate()) {
				...
				$.ajax({
					...
					success: function (data) {
						if (data.error == false) {
							...
							document.getElementById("loyalty_point_id_form").reset();
						} else {
...
viewData = (loyalty_point_id) => {
	{
		$.ajax({
			url: BASE_URL + "loyalty_point_id/" + loyalty_point_id,
			type: "GET",
			data: { loyalty_point_id },
			dataType: "json",
